...
save_dir = os.path.join(os.getcwd(),sys.argv[9])
...
if not sys.argv[5] == 'None':
    ...
    print('wrong bulk expression information')
    sys.exit()
if not sys.argv[6] == 'None' and int(sys.argv[7]):
    sample_size_list = []
    sample_sizex = []
    total_RPKM_list = []
    for indexy in range (0,int(sys.argv[7])):
        store = pd.HDFStore(sys.argv[6]+'_'+str(indexy)+'.h5')#    # scRNA-seq expression data                        )#
        rpkm = store['RPKMs']
        store.close()
...
for i in range(len(gene_pair_index)-1):   #### many sperations
    ...
    for gene_pair in gene_pair_label_array[start_index:end_index]: ## each speration
        ...
        if not sys.argv[1] == 'None': ## have bulk data
        ...
        if not sys.argv[2] == 'None': ##
            ...
            x_gene = log10(total_RPKM[h_gene_list[y_gene_name]] + 10 ** -2)
            datax = concatenate((x_tf[:, newaxis], x_gene[:, newaxis], samples[:, newaxis]), axis=1)
            H, edges = histogramdd(datax, bins=(8, 8, int(sys.argv[7])))
