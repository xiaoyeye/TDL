...
############################### model
    save_dir = os.path.join(os.getcwd(),str(test_indel) + 'Xlr0005_KEGG_3d_conv_dep_NT_p600_e' + str(epochs))
    if not os.path.isdir(save_dir):
        os.makedirs(save_dir)
    seq = Sequential()
    seq.add(Conv3D(filters=64, kernel_size=(3, 3, 3),activation='relu', input_shape=(6, 8, 8, 1),padding='same', data_format='channels_last')) ## the shape of 3D NEPDF (6,8,8,1)
    seq.add(Conv3D(filters=64, kernel_size=(3, 3, 3),
                   activation='relu',#input_shape = (9,8,8,1),
                   padding='same', data_format='channels_last'))
    #seq.add(MaxPooling3D(pool_size=(2, 2,2),padding='same',data_format='channels_last'))
    seq.add(Dropout(0.5))
    seq.add(Conv3D(filters=128, kernel_size=(3, 3, 3),activation='relu',padding='same', data_format='channels_last'))
    seq.add(Conv3D(filters=128, kernel_size=(3, 3, 3),
                   activation='relu',
